...
enum class StorageStrategy { stack, dynamic };
...
private:
    ...
    auto uncheckedIndex(size_type row, size_type column) const noexcept { return row * columnCount() + column; }
...
public:
    ...
    explicit constexpr Matrix(Arguments&&... arguments) noexcept(
        sizeof(std::enable_if_t<(M == 1 || N == 1) && M * N >= 2 && sizeof...(Arguments) == M * N &&
        ...
        if constexpr (storageStrategy == StorageStrategy::stack) {
            ...
            (..., (storage_.data.push_back(arguments)));
        }
    }
    ...
    auto& operator()(size_type row, size_type column) { return storage_.data[checkedIndex(row, column)]; }

    const auto& operator()(size_type row, size_type column) const { return storage_.data[checkedIndex(row, column)]; }
    ...
    auto rowCount() const noexcept { return M; }

    auto columnCount() const noexcept { return N; }
    ...
    auto begin() const { return storage_.data.cbegin(); }

    auto end() const { return storage_.data.cend(); }

    auto cbegin() const { return storage_.data.cbegin(); }

    auto cend() const { return storage_.data.cend(); }
...
auto crossProduct(const Matrix<Type, M, N>& a, const Matrix<Type, M, N>& b) {
    return Matrix<Type, M, N>{a.y() * b.z() - b.y() * a.z(), b.x() * a.z() - a.x() * b.z(),
                              a.x() * b.y() - b.x() * a.y()};
}
